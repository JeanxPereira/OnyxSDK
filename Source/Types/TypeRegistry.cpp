...
void TypeRegistry::RegisterByTypeId(std::unique_ptr<ITypeHandler> handler) {
    ...
    ONYX_LOGF_INFO("[TypeRegistry] Registered handler: %s", raw->GetName());
}
...
void TypeRegistry::RebuildIndexLocked() const {
    ...
    for (ITypeHandler* h : m_allHandlers) {
        ...
        if (idKey == 0) {
            ONYX_LOGF_WARN("[TypeRegistry] Handler '%s' has no TypeId (catalog not seeded?)",
                     h->GetName());
            continue;
        }
        ...
        if (!inserted) {
            ONYX_LOGF_WARN("[TypeRegistry] TypeId=%u claimed by both '%s' and '%s'; keeping '%s'",
                     idKey, it->second->GetName(), h->GetName(), it->second->GetName());
            continue;
        }

        ONYX_LOGF_DEBUG("[TypeRegistry] Indexed %s -> TypeId=%u", h->GetName(), idKey);
    }

    m_indexDirty = false;
    ONYX_LOGF_INFO("[TypeRegistry] Indexed %zu of %zu handlers by TypeId",
