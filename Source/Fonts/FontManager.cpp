...
static float       s_currentFontSize  = 14.0f;   // reference px, before UI scale
...
int FindFontIndex(const std::string& path) {
    // An empty query means "the user never chose a font". It must NOT match the
    // ProggyClean entry, whose path is also empty -- that made every fresh
    // config resolve to the bitmap default instead of falling through to
    // DefaultFontIndex(), and then persist that choice back to onyx.toml.
    if (path.empty())
        return -1;
...
void BuildAtlas(int fontIndex, float fontSizePx) {
    ...
    // fontSizePx is the reference ("logical") size. Do NOT pre-multiply it by
    // the UI scale here: since 1.92 ImGui rasterises glyphs on demand at
    // FontSizeBase * FontScaleMain * FontScaleDpi, and ScaleManager owns those
    // two factors. Baking the scale in as well would double-apply it.
    ...
    if (!s_iconCfg.path.empty() && std::filesystem::exists(s_iconCfg.path)) {
        ...
        // Minimum advance only. Setting the maximum too clamps the glyph's
        // advance without moving its ink, so any SF Symbol whose natural
        // advance is wider than iconSize kept drawing past the box ImGui
        // measures -- which is why icons sat visibly right of centre inside
        // square icon buttons. ImGui scales both values with the font size.
    ...
    // Drive the *drawn* size too. Since 1.92 the size ImGui renders at is
    // style.FontSizeBase, which it seeds from the font's reference size only
    // once (`if (FontSizeBase <= 0)`) and then never revisits -- so rebuilding
    // the atlas at a new size changed nothing on screen. Worse, the icon merge
    // scales GlyphOffset/GlyphMinAdvanceX by (drawn size / reference size), so
    // a reference that no longer matched made the icons drift as the slider
    // moved.
    ImGui::GetStyle().FontSizeBase = fontSizePx;
