...
public:
    using Factory = std::function<std::shared_ptr<Viewers::IDocumentContent>(const Domain::AssetEntry&, Domain::AssetContainer&)>;

    ViewerRegistry();

    bool CanHandle(Types::TypeId typeId) const;
    std::shared_ptr<Viewers::IDocumentContent> Open(const Domain::AssetEntry& entry, Domain::AssetContainer& wad) const;
